...
*/
	...
	Almight.Stage.prototype = {
		...
		layerCopy: function(page) {
			...
			// ベースレイヤーをコピー
			this.layerCopyTo(this[origin].base, this[page].base, page);

			// 前景レイヤーをコピー
			for(i=0;i<this.fore.layers.length;i++) {
				this.layerCopyTo(this[origin].layers[i], this[page].layers[i], page);
			}

			// メッセージレイヤーをコピー
			for(i=0;i<this.fore.messages.length;i++) {
				this.layerCopyTo(this[origin].messages[i], this[page].messages[i], page);
			}
		},
		 ...
		 */
		layerCopyTo: function(origin, to, page) {
		...
		*/
		...
		typeViewer: {
			...
			volume: function(volume) {
				...
				this.volume = volume;
				this.fade_element.volume = volume * this.gvolume;
				Titanium.App.fireEvent('setVolumeSound', { name: self.type, volume: volume * this.gvolume });
			},
...
*/
	...
	Almight.MessageLayer.prototype = {
		messageLayerInitialize: function(width, height, config){
			this.type = 'message';
		...
		initLineLayer: function() {
			...
			this.lineLayer = $('<div/>').addClass('almight-layer-line').css({
				...
				lineHeight: ((this.prop.style.linespacing || this.prop.defstyle.linespacing) + (this.prop.font.size || this.prop.deffont.size)) + 'px',
		...
		*/
		setFonts: function(params) {
			...
			if(params.shadowcolor !== undefined) this.prop.font.shadowcolor =  Almight.Util.rgba(params.shadowcolor);
			if(params.shadowpos !== undefined) this.prop.font.shadowpos = params.shadowpos;
			if(params.shadow === true || params.shadowcolor !== undefined || params.shadowpos !== undefined) {
			...
			if(params.shadow === 'default') delete this.prop.font.textShadow;
		...
		*/
		setDefStyles: function(params) {
			...
			if(params.linespacing !== undefined) this.prop.defstyle.linespacing = params.linespacing;
 ...
 * システム操作
	...
	*/
	hidemessage: function() {
		...
		this.almight.stage.canClick = false;
 ...
 * メッセージ操作
	...
	* @params {Number} [linespacing] 行間を設定
	...
	*/
	endhact: function() {
		...
		if(this.hactexp !== null && this.hactexp !== undefined) {
			$.data($('#almight-history-box span:last')[0], 'hact', this.hactexp);
			$('#almight-history-box span:last').addClass('hactlink').on('tap', function() {
	...
	*/
	copylay: function(params) {
		...
		if(src.type !== dest.type) {
			...
			this.stage.layerCopyTo(src, dest, to);
		}
		...
		*/
		load: function(src) {
			...
			if(this.scenario_name in this.scenario) {
				if(this.almight.debug) console.log('============================\nシナリオをロード: '+ this.scenario_name + ' (cache)\n============================');
				return df.resolve();
...
'use strict';
	...
	Almight.Core.prototype = {
		...
		initialize: function(config) {
			...
			if(this.config.filepath) {
				promises.push((function() {
					return $.ajax({
						url: Almight.resourcePath + 'filepath',
						type: 'GET',
						dataType: 'text'
		...
		allocateBaseLayer: function() {
			this._fore.base = new Almight.GraphicLayer(this.config.width, this.config.height);
			this._fore.base.setVisible(true);
			this._fore.base.type = 'base';
			this.stage.add(this._fore.base, 'fore', 'base');

			this._back.base = new Almight.GraphicLayer(this.config.width, this.config.height);
		...
		allocateLayers: function(count) {
			...
			if(this._fore.layers.length > count) {
				...
				for(i=this._fore.layers.length;i<count;i++) {
					this._fore.layers[i] = new Almight.GraphicLayer(this.config.width, this.config.height);
					this.stage.add(this._fore.layers[i], 'fore', i);
					this._back.layers[i] = new Almight.GraphicLayer(this.config.width, this.config.height);
		...
		allocateMessageLayers: function(count) {
			...
			if(this._fore.messages.length > count) {
				...
				for(i=this._fore.messages.length;i<count;i++) {
					this._fore.messages[i] = new Almight.MessageLayer(this.config.width, this.config.height, this.config.messages);
					this.stage.add(this._fore.messages[i], 'fore', 'message' + i);
					this._back.messages[i] = new Almight.MessageLayer(this.config.width, this.config.height, this.config.messages);
		...
		buttonsEvent: function() {
			...
			$('.almight-link, .almight-button, .almight-choices').each(function(){
				$(this).on('mouseover', function(){
					...
					$('.almight-link, .almight-button, .almight-choices').off('mouseover mouseout mousedown mouseup click');
					$('.almight-link').removeClass('almight-link');
